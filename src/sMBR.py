...
group_num=len(src)
assert group_num==(len(hypo)//(k)), "group_num: {}, hypo: {}, k: {}".format(group_num,len(hypo),k)
...
assert len(data_list)==(group_num*src_num*k), "len(data_list): {}, group_num: {}, src_num: {}, k: {}".format(len(data_list),group_num,src_num,k)
print("successfully load data list")

model_path=download_model(args.qe_model,saving_directory="/raid/lyu/comet_model")
