...
if args.use_gpu:
    if gpu_num==1:
        translator = ctranslate2.Translator(model_path, device="cuda",device_index=[0],compute_type=args.compute_type)
    elif gpu_num==2:
        translator = ctranslate2.Translator(model_path, device="cuda",device_index=[0,1],compute_type=args.compute_type)
    elif gpu_num==4:
        translator = ctranslate2.Translator(model_path, device="cuda",device_index=[0,1,2,3],compute_type=args.compute_type)
else:
...
with open(src_file, "r", encoding="utf-8") as fsrc:
    ...
    if spm_model is not None:
        ...
        for line in tqdm(lines,desc="tokenize with fastbpe",total=len(lines)):
...
print("successfully load src file with {} lines".format(len(src)))
batch_size=32
results=[]
for i in tqdm(range(len(src)//batch_size+1),desc="translate",total=len(src)//batch_size+1):
    if args.use_gpu:
        if args.sentence_batch:
            results.extend(translator.translate_batch(src[i*batch_size:(i+1)*batch_size],batch_type="examples", max_batch_size=args.batch_size,beam_size=args.beam_size,disable_unk=True,return_scores=True,sampling_topk=args.topk,sampling_topp=args.topp,num_hypotheses=args.num_hypotheses))
        else:
            results.extend(translator.translate_batch(src[i*batch_size:(i+1)*batch_size],batch_type="tokens", max_batch_size=args.batch_size,beam_size=args.beam_size,disable_unk=True,return_scores=True,sampling_topk=args.topk,sampling_topp=args.topp,num_hypotheses=args.num_hypotheses))
    else:
        results.extend(translator.translate_batch(src[i*batch_size:(i+1)*batch_size],batch_type="tokens", max_batch_size=500,beam_size=args.beam_size,disable_unk=True,return_scores=True,sampling_topk=args.topk,sampling_topp=args.topp,num_hypotheses=args.num_hypotheses))
